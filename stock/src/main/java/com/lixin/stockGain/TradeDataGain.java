...
public class TradeDataGain {
     ...
     * @return 实时
     */
    public RealTimeStockInfo realTimeSina(String StockCode){
        String sz000078 = API.ME(API.HOST.sina).addReqUrl(StockCode).GET();
        return RealTimeStockInfo.build(sz000078);
    }
    ...
    public void testSina() throws InterruptedException {
        for (int i = 0; i < 10000 ; i++) {
            RealTimeStockInfo realTimeStockInfo = realTimeSina("sz000078");
            System.out.println(realTimeStockInfo);
            Thread.sleep(500);
        }
